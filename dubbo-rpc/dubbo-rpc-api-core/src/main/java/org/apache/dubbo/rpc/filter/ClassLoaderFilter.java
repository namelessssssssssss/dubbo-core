...
public class ClassLoaderFilter implements Filter, BaseFilter.Listener {
    ...
    public Result invoke(Invoker<?> invoker, Invocation invocation) throws RpcException {
        ...
        if (effectiveClassLoader != null) {
            invocation.put(CommonConstants.STAGED_CLASSLOADER_KEY, stagedClassLoader);
            invocation.put(CommonConstants.WORKING_CLASSLOADER_KEY, effectiveClassLoader);
    ...
    private void resetClassLoader(Invoker<?> invoker, Invocation invocation) {
        ClassLoader stagedClassLoader = (ClassLoader) invocation.get(CommonConstants.STAGED_CLASSLOADER_KEY);
