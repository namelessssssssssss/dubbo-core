    ...
    public <T> T getProxy(Invoker<T> invoker, Class<?>[] interfaces) {
        try {
            ...
            try {
                ...
                logger.error(LoggerCodeConstants.PROXY_FAILED, "", "", "Failed to generate proxy by Javassist failed. Fallback to use JDK proxy success. " +
                    "Interfaces: " + Arrays.toString(interfaces), fromJavassist);
                return proxy;
            } catch (Throwable fromJdk) {
                logger.error(LoggerCodeConstants.PROXY_FAILED, "", "", "Failed to generate proxy by Javassist failed. Fallback to use JDK proxy is also failed. " +
    ...
    public <T> Invoker<T> getInvoker(T proxy, Class<T> type, URL url) {
        try {
            ...
            try {
                ...
                logger.error(LoggerCodeConstants.PROXY_FAILED, "", "", "Failed to generate invoker by Javassist failed. Fallback to use JDK proxy success. " +
                    "Interfaces: " + type, fromJavassist);
                // log out error
                return invoker;
            } catch (Throwable fromJdk) {
                logger.error(LoggerCodeConstants.PROXY_FAILED, "", "", "Failed to generate invoker by Javassist failed. Fallback to use JDK proxy is also failed. " +
