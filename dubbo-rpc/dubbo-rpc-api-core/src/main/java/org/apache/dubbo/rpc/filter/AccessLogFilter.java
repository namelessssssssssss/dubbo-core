...
public class AccessLogFilter implements Filter {
    ...
    public Result invoke(Invoker<?> invoker, Invocation inv) throws RpcException {
        ...
        try {
            ...
            logger.warn(LoggerCodeConstants.CONFIG_FILTER_VALIDATION_EXCEPTION, "", "", "Exception in AccessLogFilter of service(" + invoker + " -> " + inv + ")", t);
        }
        try {
            return invoker.invoke(inv);
        } finally {
    ...
    private void writeLogSetToFile(String accessLog, Queue<AccessLogData> logSet, boolean isFixedPath) {
        try {
            if (ConfigUtils.isDefault(accessLog)) {
                ...
                if (isFixedPath) {
                    logger.warn(LoggerCodeConstants.VULNERABILITY_WARNING, "Change of accesslog file path not allowed. ", "", "Will write to the default location, \" +\n" +
                    ...
                    logger.warn(LoggerCodeConstants.VULNERABILITY_WARNING, "Accesslog file path changed to " + accessLog + ", be aware of possible vulnerabilities!", "", "");
