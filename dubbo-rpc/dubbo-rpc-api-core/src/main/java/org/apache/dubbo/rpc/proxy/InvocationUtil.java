    ...
    public static Object invoke(Invoker<?> invoker, RpcInvocation rpcInvocation) throws Throwable {
        ...
        if (ProfilerSwitch.isEnableSimpleProfiler()) {
            ...
            try {
                ...
                Long timeout = RpcUtils.convertToNumber(rpcInvocation.getObjectAttachmentWithoutConvert(CommonConstants.TIMEOUT_KEY));

                if (timeout == null) {
                    timeout = (long) url.getMethodPositiveParameter(rpcInvocation.getMethodName(),
                        CommonConstants.TIMEOUT_KEY,
                ...
                if ((usage / (1000_000L * ProfilerSwitch.getWarnPercent())) > timeout) {
                    ...
                    logger.warn(LoggerCodeConstants.PROXY_TIMEOUT_REQUEST, "", "", String.format(
