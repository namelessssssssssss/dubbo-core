...
public class GenericImplFilter implements Filter, Filter.Listener {
    ...
    public Result invoke(Invoker<?> invoker, Invocation invocation) throws RpcException {
        ...
        if (isCallingGenericImpl(generic, invocation)) {
            ...
            if (RpcUtils.isReturnTypeFuture(invocation)) {
                invocation2.setMethodName(CommonConstants.$INVOKE_ASYNC);
            } else {
                invocation2.setMethodName(CommonConstants.$INVOKE);
            }
            invocation2.setParameterTypes(GENERIC_PARAMETER_TYPES);
            invocation2.setParameterTypesDesc(CommonConstants.GENERIC_PARAMETER_DESC);
                ...
                try {
                    ...
                    if (targetException != null) {
                        try {
                            ...
                            logger.warn(LoggerCodeConstants.COMMON_REFLECTIVE_OPERATION_FAILED, "", "", e.getMessage(), e);
                        }
    ...
    private boolean isCallingGenericImpl(String generic, Invocation invocation) {
        return ProtocolUtils.isGeneric(generic)
            && (!CommonConstants.$INVOKE.equals(invocation.getMethodName()) && !CommonConstants.$INVOKE_ASYNC.equals(invocation.getMethodName()))
            && invocation instanceof RpcInvocation;
    }

    private boolean isMakingGenericCall(String generic, Invocation invocation) {
        return (invocation.getMethodName().equals(CommonConstants.$INVOKE) || invocation.getMethodName().equals(CommonConstants.$INVOKE_ASYNC))
