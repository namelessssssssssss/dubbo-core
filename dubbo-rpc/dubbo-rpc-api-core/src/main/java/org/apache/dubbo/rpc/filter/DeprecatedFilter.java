    ...
    public Result invoke(Invoker<?> invoker, Invocation invocation) throws RpcException {
        ...
        if (!LOGGED.contains(key)) {
            ...
            if (invoker.getUrl().getMethodParameter(invocation.getMethodName(), DEPRECATED_KEY, false)) {
                LOGGER.error(LoggerCodeConstants.COMMON_UNSUPPORTED_INVOKER, "", "", "The service method " + invoker.getInterface().getName() + "." + getMethodSignature(invocation) + " is DEPRECATED! Declare from " + invoker.getUrl());
            }
        }
