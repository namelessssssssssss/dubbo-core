...
public class ProfilerServerFilter implements Filter, BaseFilter.Listener {
    ...
    private void addAdaptiveResponse(Result appResponse, Invocation invocation) {
        ...
        if (StringUtils.isNotEmpty(adaptiveLoadAttachment)) {
            ...
            sb.append(CommonConstants.COMMA_SEPARATOR).append("load:").append(operatingSystemMXBean.getSystemLoadAverage() * 100 / operatingSystemMXBean.getAvailableProcessors() );
    ...
    private void dumpIfNeed(Invoker<?> invoker, Invocation invocation, ProfilerEntry profiler) {
        Long timeout = RpcUtils.convertToNumber(invocation.getObjectAttachmentWithoutConvert(CommonConstants.TIMEOUT_KEY));

        if (timeout == null) {
            timeout = (long) invoker.getUrl().getMethodPositiveParameter(invocation.getMethodName(), CommonConstants.TIMEOUT_KEY, CommonConstants.DEFAULT_TIMEOUT);
        }
        ...
        if (((usage / (1000_000L * ProfilerSwitch.getWarnPercent())) > timeout) && timeout != -1) {
            ...
            logger.warn(LoggerCodeConstants.PROXY_TIMEOUT_RESPONSE, "", "",
