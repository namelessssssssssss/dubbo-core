...
public abstract class AbstractProtocol implements Protocol, ScopeModelAware {
    ...
    protected void loadServerProperties(ProtocolServer server) {
        ...
        server.getAttributes().put(CommonConstants.SHUTDOWN_WAIT_KEY, serverShutdownTimeout);
    }

    protected int getServerShutdownTimeout(ProtocolServer server) {
        return (int) server.getAttributes().getOrDefault(CommonConstants.SHUTDOWN_WAIT_KEY, CommonConstants.DEFAULT_SERVER_SHUTDOWN_TIMEOUT);
    }
        ...
        for (Invoker<?> invoker : invokers) {
            if (invoker != null) {
                try {
                    ...
                    logger.warn(LoggerCodeConstants.PROTOCOL_FAILED_DESTROY_INVOKER, "", "", t.getMessage(), t);
                }
            }
        }
        invokers.clear();
    ...
    protected void optimizeSerialization(URL url) throws RpcException {
        String className = url.getParameter(CommonConstants.OPTIMIZER_KEY, "");
