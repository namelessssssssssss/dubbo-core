...
public abstract class AbstractProxyFactory implements ProxyFactory {
    ...
    public <T> T getProxy(Invoker<T> invoker, boolean generic) throws RpcException {
        ...
        if (StringUtils.isNotEmpty(config)) {
            String[] types = CommonConstants.COMMA_SPLIT_PATTERN.split(config);
        ...
        try {
            ...
            if (generic) {
                ...
                logger.error(LoggerCodeConstants.PROXY_UNSUPPORTED_INVOKER, "", "", "Error occur when creating proxy. Invoker is in generic mode. Trying to create proxy without real interface class.", t);
                return getProxy(invoker, interfaces.toArray(new Class<?>[0]));
            } else {
                throw t;
            }
