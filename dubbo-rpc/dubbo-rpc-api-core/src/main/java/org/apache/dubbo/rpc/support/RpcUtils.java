...
public class RpcUtils {
    ...
    public static Class<?> getReturnType(Invocation invocation) {
        try {
            ...
            logger.warn(LoggerCodeConstants.COMMON_REFLECTIVE_OPERATION_FAILED, "", "", t.getMessage(), t);
        }
        return null;
    }
    ...
    public static String getMethodName(Invocation invocation) {
        if (CommonConstants.$INVOKE.equals(invocation.getMethodName())
    ...
    public static boolean isGenericAsync(Invocation inv) {
        return CommonConstants.$INVOKE_ASYNC.equals(inv.getMethodName());
    }

    // check parameterTypesDesc to fix CVE-2020-1948
    public static boolean isGenericCall(String parameterTypesDesc, String method) {
        return (CommonConstants.$INVOKE.equals(method) || CommonConstants.$INVOKE_ASYNC.equals(method)) && CommonConstants.GENERIC_PARAMETER_DESC.equals(parameterTypesDesc);
    }
    ...
    public static long getTimeout(Invocation invocation, long defaultTimeout) {
        ...
        Object genericTimeout = invocation.getObjectAttachmentWithoutConvert(CommonConstants.TIMEOUT_ATTACHMENT_KEY);
        if (genericTimeout == null) {
            genericTimeout = invocation.getObjectAttachmentWithoutConvert(CommonConstants.TIMEOUT_ATTACHMENT_KEY_LOWER);
        }
        if (genericTimeout != null) {
    ...
    public static int calculateTimeout(URL url, Invocation invocation, String methodName, long defaultTimeout) {
        Object countdown = RpcContext.getClientAttachment().getObjectAttachment(CommonConstants.TIME_COUNTDOWN_KEY);
        ...
        if (countdown == null) {
            if (url != null) {
                ...
                if (url.getMethodParameter(methodName, CommonConstants.ENABLE_TIMEOUT_COUNTDOWN_KEY, false)) {
                    // pass timeout to remote server
                    invocation.setObjectAttachment(CommonConstants.TIMEOUT_ATTACHMENT_KEY, timeout);
                }
            }
        } else {
        ...
        invocation.getObjectAttachments().remove(CommonConstants.TIME_COUNTDOWN_KEY);
        return timeout;
    }
